...
( function( blocks, blockEditor, element, components ) {
  ...
  if ( !window.PLUGIN_BOILERPLATE_gutenberg_blocks ) { return; }

  Object.values(window.PLUGIN_BOILERPLATE_gutenberg_blocks).forEach( function( block ) {
    ...
    registerBlockType(block.name, {
      ...
      edit: function (props) {
        return (
          createElement('div', {className: 'plugin-boilerplate-shortcode-block'},

            createElement(Button, {
              'data-modal-id': block.modal_id,
              'data-gutenberg-id': block.name,
              className: 'is-secondary plugin-boilerplate-shortcode-button',
              onClick: function () {
                window.PLUGIN_BOILERPLATE_gutenberg_props = props;
              },
            }, block.button_title ),
